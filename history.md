...
### [2026-05-04] 스캔 모드 일괄 번역 기능 구현 및 80개 언어 현지화 (v2.1.3+118)
...
- **UI/UX Enhancement**: OCR 인식 완료 후 사용자가 번역 방식(일괄/개별)을 선택할 수 있는 세련된 다이얼로그를 추가했습니다. 또한 추출된 텍스트 목록 상단에 '전체 번역' 버튼을 배치하여 사용성을 높였습니다.
- **Bug Fix (Navigation)**: 일괄 번역 선택 시 잔여 횟수가 부족하여 경고 창이 뜰 때, `Navigator.pop()`이 중복 호출되어 스캔 화면 자체가 닫히고 검은 화면이 노출되던 내비게이션 버그를 수정했습니다. (`v2.1.3+118`)
- **80+ L10n Completion**: 신규 UI 키(번역 방식 선택, 한도 부족 안내 등)를 80개 이상의 모든 지원 언어에 대해 자동 번역 및 적용 완료했습니다.
- **Code Integrity**: `flutter analyze` 정적 분석 결과 **0 issues**를 달성했으며, 불필요한 임시 파일 정리 및 버전 범프(`v2.1.3+118`)를 완료했습니다.
