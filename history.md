...
## 📅 [2026-02-08 01:10:00] Phase 76.5 UI/UX Polish - Online Library Refactor

### ✅ 태스크 (Task)
- [x] **`Online Library` 통합 다이얼로그 구현**: 단어/문장/대화 탭을 분리하여 사용자 경험 개선.
- [x] **자료 필터링 기능 추가**: `materials_v3.json`의 `category` 필드를 기반으로 자료 필터링 적용.
- [x] **자료 임포트 후 자동 모드 전환 및 선택**: 자료 임포트 완료 시 해당 모드로 자동 전환 및 선택되도록 구현.
- [x] **국제화(L10n) 업데이트**: `tabConversation` 키 추가 및 80개국 언어 파일 동기화 완료.
...
- [x] **중복 정의 제거**: `HomeScreen` 내 `_showMaterialSelectionDialog` 함수가 중복 정의되어 발생한 컴파일 에러 수정.
- [x] **누락된 키 추가**: `HelpDialog`에서 사용되었으나 ARB 파일에 누락된 `startTutorial` 로컬라이제이션 키 추가.
- [x] **코드 정리**: 잘못된 병합으로 인해 남겨진 레거시 함수 헤더 삭제.

### 📝 워크스루 (Walkthrough)
- **원인**: UI/UX 폴리싱 작업 중 `multi_replace` 도구 사용 오류로 인해 함수 선언부가 중복 삽입되는 문제가 발생했습니다.
- **조치**: 중복된 외부 함수 선언부를 제거하고 내부의 올바른 로직만 남겨 정상적인 클래스 구조를 복구했습니다.
