...
### [2026-04-11] 안드로이드 구버전 호환성 근본 해결 및 UI 구조 최적화 (v2.0.0+113)

- **Database Compatibility (Legacy Android)**: 특정 안드로이드 구형 기기에서 FFI 엔진 초기화 실패로 학습 카드가 생성되지 않던 문제를 해결하기 위해, 안정적인 **네이티브 SQLite 드라이버**로 전격 복구했습니다. 
- **JSON1 Dependency Removal**: 구형 시스템 SQLite에서 지원하지 않는 `json_each`, `json_extract` 함수 의존성을 쿼리에서 완전히 제거하고 표준 SQL(`LIKE`) 방식으로 재작성하여 100% 호환성을 확보했습니다.
- **Reverse Migration**: v112 버전에서 문서 폴더로 이동되었던 데이터베이스를 유실 없이 다시 시스템 표준 폴더로 되돌리는 복구 로직을 구현했습니다.
- **UI Optimization (Mode 2/3)**: 사용자의 요청에 따라 복습 및 발음 연습 모드의 상단 컨트롤을 2단 구조(1행: 토글+도움말, 2행: 언어 전환)로 개편하여 조작성과 시인성을 개선했습니다.
- **Quality Assurance**: `flutter analyze` 정적 분석 결과 **0 issues**를 달성하여 배포 안정성을 최종 검증했습니다.
