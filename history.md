...
### [2026-03-09] 온보딩 경험 개선 및 80개 언어 지원 강화 (v1.9.0)

- **Welcome Banner**: 신규 사용자가 앱의 목적을 오해하지 않도록 메인 화면 상단에 세련된 그라데이션 디자인의 '환영 배너'를 추가했습니다.
- **Onboarding Logic**: `SharedPreferences`를 활용해 사용자가 배너를 닫으면 다시 나타나지 않도록 구현했습니다.
- **80+ Language Support**: 앱이 지원하는 80개 이상의 모든 언어(ARB 파일)에 대해 환영 문구 자동 번역 및 업데이트를 완료했습니다. 특히 **"80개 이상의 언어 지원"**이라는 핵심 장점을 각국의 언어로 부각시켰습니다.
- **Automation**: PowerShell 스크립트를 활용하여 대규모 ARB 파일을 일괄 처리하는 파이프라인을 구축하고 검증했습니다.
