...
class AppState extends ChangeNotifier {
  ...
  final SentenceRepository _sentenceRepository = SentenceRepository();
  ...
  Future<void> saveTranslation() async {
    ...
    try {
      ...
      // Check if duplicate source exists
      // For similar sources, we might want to let user choose (logic exists in searchSimilarSources)
      ...
      if (existingGroupId != null) {
        ...
        debugPrint('[AppState] Found existing group: $groupId');
      } else {
        // Create new Source Sentence
        // Generate a random Group ID (Temporary solution until we have sequences or Group table)
        ...
        debugPrint('[AppState] Created new group: $groupId');
      }
      
      // 2. Handle Target Sentence
      // Check if target already exists in this group
      // (Optimization: we could check this earlier)
      
      // Create Target Sentence (Always create for now, DB handles dedupe if strictly enforced,
      // but schema allows multiple same-text if diff group. Here same group.)
      // Actually we should check if (groupId, langCode) already has a sentence?
      // For now, let's just add it. The Repository addSentence deals with insert.
      // Wait, if we multiple translations for same group?
      // Schema: unique(group_id, lang_code, text) might be good.
      ...
      // We need to insert into user_library
      // We don't have a UserLibraryRepository yet, so use Supabase directly or create one?
      // Using SupabaseService directly for simplicity as per plan.
      ...
      if (userId != null) {
         try {
           await SupabaseService.client.from('user_library').insert({
             ...
             // 'review_stats': default
           });
         } catch (e) {
           // Ignore duplicate key error (already in library)
      ...
      // Reload logic (Stubbed for now, need to replace loadStudyRecords)
