...
- **온디바이스 스마트 병합 전환 (Phase 75.9)**: 복잡한 오프라인 병합 스크립트(`merge_materials.ps1`) 의존성을 제거하고, 개별 언어 파일 임포트 시 `subject` 기반으로 기기 내에서 데이터를 동적으로 통합하는 스마트 매핑 엔진 도입.
- **채팅 UI 실시간 언어 연동**: 채팅 화면의 언어 스위치 조작 시 대화 데이터가 실시간으로 재성립되어 다국어 대화를 매끄럽게 지원.
- **대화 임포트 정합성 해결**: 대화 자료 임포트 시 `group_id` 매핑 불일치로 인해 내용이 보이지 않던 버그를 정밀 매핑 로직으로 해결.

## 최근 업데이트 (Recent Updates - 2026-02-07)

- **온디바이스 스마트 병합 및 동적 매핑 (Phase 75.9)**: 외부 학습 자료 임포트 시 오프라인 병합 과정 없이도 개별 언어 파일을 주제별로 자동 통합하는 로직을 구현했습니다. 이를 통해 채팅 화면에서 언어 전환 스위치 작동 시 실시간으로 다국어 연동이 가능하며, 성별/로케일 기반 TTS 엔진과의 완벽한 정합성을 확보하여 '콩글리쉬' 문제를 근본적으로 해결했습니다.
