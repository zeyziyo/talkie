...
*   **`sentences_meta`** (로컬 `sentences_meta` 대응)
...
3.  **`dialogue_participants`** (참여자 연결 백업)
    *   `dialogue_id` (UUID FK NN): `dialogue_groups.id` 참조.
    *   `participant_id` (TEXT NN): 사용자의 `participants.id` 참조.
    *   `user_id` (UUID FK NN): RLS용 소유자 ID.
    *   `joined_at` (TIMESTAMP WITH TIME ZONE NN).
    *   **PK (Primary Key)**: `(dialogue_id, participant_id)`.

4.  **`dialogue_messages`** (대화 내역 백업)
    *   `id` (BIGINT PK GENERATED BY DEFAULT AS IDENTITY).
    *   `session_id` (UUID FK NN): `dialogue_groups.id` 참조.
    *   `user_id` (UUID FK NN): RLS용 소유자 ID.
    *   `speaker` (TEXT NN): `participants.id` 참조 (화자 ID 저장).
    *   `content` (TEXT NN): 메시지 내용.
...
## 9. 출시 준비 및 국제화 사양 (v14.6+)

### 9.1 국제화 지원 범위
- **지원 언어**: 한국어(ko), 영어(en)를 포함한 **전 세계 80개 이상의 모든 주요 언어** 완벽 대응
- **로컬라이징 대상**: UI 라벨, 도움말, 에러 메시지, 튜토리얼 툴팁, 참가자 관리 인터페이스 (전 언어 자동 동기화 적용)

### 9.2 도움말 시스템 (v14.6+)
- **JSON 가져오기 규격**: `dialogues` 배열과 `participants` (ID 기반) 정보를 포함한 최신 규격 유지
- **Managed Participants 가이드**: 대화 상대별 언어/성별 설정법 및 기록 보존 원리 설명 보강
- **데이터 무결성 안내**: 화자 이름 변경 시에도 대화 기록이 유지되는 ID 매핑 체계 명시
