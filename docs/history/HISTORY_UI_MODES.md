...
### 2026-02-10: 모드 1 입력 경험 혁신 (Phase 81.4)
- **자동 완성 (Autocomplete)**: 메인 입력란에 기존 데이터를 기반으로 한 실시간 제안 기능을 도입하여 중복 입력 방지 및 편의성 극대화.
- **제목 관리 (Subject Management)**: 입력 모드에서 저장할 자료집 제목을 즉시 선택하거나 생성할 수 있는 Autocomplete 필드 배치.
- **라벨 고도화**: 자료집 선택 필드에 현재 모드(단어/문장)에 부합하는 지역화된 힌트 텍스트 적용.

### 2026-02-10: 가시성 복구 및 검색 통합 (Phase 81.2 - 81.3)
- **모국어 검색**: 목표어 번역 유무와 관계없이 모국어 텍스트만으로 자료를 찾고 로딩할 수 있도록 검색 로직 통합.
- **성능 최적화**: 자료 저장 직후 즉시 복습 모드(Mode 2) 목록에 반영되도록 `notifyListeners` 및 데이터 로딩 로직 튜닝.
